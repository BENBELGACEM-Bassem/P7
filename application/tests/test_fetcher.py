...
from ..grandpy.utils.config import (Scrap, GmgeoParams, WikiParams)

from ..grandpy.utils.parser import parse

from ..grandpy.utils.fetcher import(GmGeoApi, WikiApi)
...
def test_get_adress_with_ok_status(mock_gmgeo_ok_status):
    google_request_instance = GmGeoApi("dummy user request")
    address = google_request_instance.get_address()
...
def test_get_latitude_longitude_with_ok_status(mock_gmgeo_ok_status):
    google_request_instance = GmGeoApi("dummy user request")
    gps_coordinates = google_request_instance.get_latitude_longitude()
